...
class HKStockAnalyzer:
    ...
    def __init__(self, code: str, use_ai: bool = True, market_context: Dict = None, api_source: str = None):
        self.code = code

        # Detect region based on ticker format (HK = digits only, US = letters)
        self.region = "HK" if code.isdigit() else "US"

        # Use api_source if provided, otherwise auto-detect
        if api_source == "futu":
            # User explicitly chose Futu API
            self.futu = FutuClient() if FUTU_AVAILABLE else None
            self.itick = None
            self.yahoo = None
            if not self.futu:
                print("Warning: Futu not available, falling back to iTick")
                self.futu = None
                self.itick = ITickClient(get_next_itick_token(), region=self.region)
        elif api_source == "itick":
            # User explicitly chose iTick API
            self.futu = None
            self.itick = ITickClient(get_next_itick_token(), region=self.region)
            self.yahoo = None
        elif self.region == "HK" and FUTU_AVAILABLE:
            # Auto-detect: Use Futu for HK stocks
            self.futu = FutuClient()
            self.itick = None
            self.yahoo = None
        else:
            # Auto-detect: Use iTick for US stocks or when Futu not available
            self.futu = None
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python stock_analysis_v2.py <STOCK_CODES> [options]")
        print("\nExamples:")
        print("  python stock_analysis_v2.py us                    # Analyze top US stocks")
        print("  python stock_analysis_v2.py hk                    # Analyze top HK stocks")
        print("  python stock_analysis_v2.py us --top=10           # Analyze top 10 US stocks")
        print("  python stock_analysis_v2.py hk --top=10           # Analyze top 10 HK stocks")
        print("  python stock_analysis_v2.py 700,9988 --api=futu     # Use Futu API for HK stocks")
        print("  python stock_analysis_v2.py aapl,msft --api=itick # Use iTick API for US stocks")
        print("  python stock_analysis_v2.py all --api=itick       # Use iTick for all stocks")
        print("  python stock_analysis_v2.py us --signals          # Show only BUY/SELL signals")
        print("\nAPI Options:")
        print("  --api=futu   Use Futu API (HK/US market data)")
        print("  --api=itick Use iTick API (HK/US market data)")
        print("  (default: auto-detect based on stock region)")
    ...
    # Parse --api flag for API selection (futu or itick)
    api_source = None  # None = auto-detect, "futu" = use Futu, "itick" = use iTick
    for i, arg in enumerate(sys.argv):
        if arg.startswith("--api="):
            api_source = arg.split("=")[1].lower().strip()
            if api_source not in ["futu", "itick"]:
                print("Invalid API. Use --api=futu or --api=itick")
                sys.exit(1)
    ...
    # Use Futu for HK, iTick for US (or based on --api flag)
    if api_source == "futu":
        # User explicitly selected Futu API
        futu = FutuClient() if FUTU_AVAILABLE else None
        itick = None
        if not futu:
            print("Warning: Futu API not available. Falling back to iTick.")
            itick = ITickClient(get_next_itick_token(), region=region)
            api_source = "itick"
        else:
            print("Using Futu API (user selected)")
    elif api_source == "itick":
        # User explicitly selected iTick API
        futu = None
        itick = ITickClient(get_next_itick_token(), region=region)
        print("Using iTick API (user selected)")
    elif region == "HK" and FUTU_AVAILABLE:
        # Auto-detect: Use Futu for HK
        futu = FutuClient()
        itick = None
        print("Using Futu API (auto-detect for HK)")
    else:
        # Auto-detect: Use iTick for US or when Futu not available
        futu = None
        itick = ITickClient(get_next_itick_token(), region=region)
        print("Using iTick API (auto-detect)")
    ...
    for i, code in enumerate(codes):
        ...
        # Run analysis with pre-fetched market context and API source
        analyzer = HKStockAnalyzer(code, market_context=market_context, api_source=api_source)
